...
export default function Auth() {
    ...
    const handleGoogleLogin = async () => {
        ...
        setError('');
        try {
            // Use Google Identity Services (GSI) for One-Tap sign-in
            if (!window.google?.accounts) {
                setError('Google Sign-In לא זמין. רענן את הדף.');
                setLoading(false);
                return;
            }
            ...
            window.google.accounts.id.initialize({
                ...
                callback: async (response) => {
                    try {
                        ...
                        setError(err.response?.data?.message || 'שגיאה באימות Google');
                        setLoading(false);
                    }
                }
            });
                        ...
                        <>
                            ...
                            <form onSubmit={handleLogin} className="space-y-4">
                                <div className="relative">
                                    <FiMail className="absolute right-4 top-1/2 transform -translate-y-1/2 text-slate-400" size={18} />
                                    <input
                                        id="login-email"
                                        type="email"
                                        name="email"
                                        ...
                                        required
                                        className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl py-3.5 pr-12 pl-4 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition duration-200"
                                    />
                                </div>
                                <div className="relative">
                                    <FiLock className="absolute right-4 top-1/2 transform -translate-y-1/2 text-slate-400" size={18} />
                                    <input
                                        id="login-password"
                                        ...
                                        onChange={handleChange}
                                        required
                                        className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl py-3.5 pr-12 pl-12 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition duration-200"
                                    />
                                    <button type="button" onClick={() => setShowPassword(!showPassword)}
                                        className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-slate-200 transition">
                                ...
                                <button
                                    ...
                                    type="submit"
                                    disabled={loading}
                                    className="w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold py-3.5 rounded-xl transition duration-300 shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {loading ? <span className="inline-block w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'התחבר'}
                                </button>
                            </form>
                            ...
                            <div id="google-signin-fallback" className="mt-2 flex justify-center" />
                    ...
                    {mode === 'register' && (
                        <>
                            ...
                            <form onSubmit={handleRegister} className="space-y-4">
                                <div className="relative">
                                    <FiUser className="absolute right-4 top-1/2 transform -translate-y-1/2 text-slate-400" size={18} />
                                    <input
                                        id="register-name"
                                        type="text"
                                        name="name"
                                        ...
                                        onChange={handleChange}
                                        required
                                        className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl py-3.5 pr-12 pl-4 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition duration-200"
                                    />
                                </div>
                                <div className="relative">
                                    <FiMail className="absolute right-4 top-1/2 transform -translate-y-1/2 text-slate-400" size={18} />
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        required
                                        minLength={6}
                                        className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl py-3.5 pr-12 pl-12 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition duration-200"
                                    />
                                    <button type="button" onClick={() => setShowPassword(!showPassword)}
                                        className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-slate-200 transition">
                                        {showPassword ? <FiEyeOff size={18} /> : <FiEye size={18} />}
                                ...
                                <button
                                    ...
                                    type="submit"
                                    disabled={loading}
                                    className="w-full bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-bold py-3.5 rounded-xl transition duration-300 shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {loading ? <span className="inline-block w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'הירשם'}
                                </button>
                            </form>
                    ...
                    {mode === 'verify' && (
                        <>
                            ...
                            <form onSubmit={handleVerify} className="space-y-6">
                                ...
                                >
                                    {loading ? <span className="inline-block w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'אמת חשבון'}
                                </button>
                            </form>
                        ...
                        <>
                            ...
                            <form onSubmit={handleForgotPassword} className="space-y-4">
                                ...
                                <button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold py-3.5 rounded-xl transition duration-300 shadow-lg shadow-orange-500/25 disabled:opacity-50"
                                >
                                    {loading ? <span className="inline-block w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'שלח קוד איפוס'}
                                </button>
                            </form>
                        </>
                    )}
                        ...
                        <>
                            ...
                            <form onSubmit={handleResetPassword} className="space-y-4">
                                ...
                                <button
                                    ...
                                    className="w-full bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold py-3.5 rounded-xl transition duration-300 shadow-lg shadow-orange-500/25 disabled:opacity-50"
                                >
                                    {loading ? <span className="inline-block w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'שנה סיסמה'}
                                </button>
                            </form>
                        </>
                    )}
